...
from oioswift.common.middleware.s3api.subresource import ACL, User, \
    encode_acl, Owner, Grant
from oioswift.common.middleware.s3api.etree import fromstring
from oioswift.common.middleware.s3api.utils import mktime, S3Timestamp
...
class TestS3Obj(S3TestCase):
    ...
    def test_object_PUT_copy_headers_with_match_and_s3acl(self):
        ...
        # S3 check the bucket write permissions of the destination.
...
class TestS3ObjNonUTC(TestS3Obj):
    def setUp(self):
        self.orig_tz = os.environ.get('TZ', '')
        os.environ['TZ'] = 'EST+05EDT,M4.1.0,M10.5.0'
        time.tzset()
        super(TestS3ObjNonUTC, self).setUp()

    def tearDown(self):
        super(TestS3ObjNonUTC, self).tearDown()
